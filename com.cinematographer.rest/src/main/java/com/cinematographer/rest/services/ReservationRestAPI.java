...
public class ReservationRestAPI {
	...
	public Response reserveSeats(String payload) {
		try {
			...
			ReservationDTO reservation = JsonUtils.fromJson(payload,
					ReservationDTO.class);

			service.reserveSeats(reservation.getUsername(),
					reservation.getTitle(), reservation.getSeats());
	...
	public Response getActiveReservations(@PathParam("username") String username) {
		try {
			IReservationService service = getReservationService();
			Collection<Reservation> reservations = service
					.getActiveReservations(username);
	...
	public Response payReservation(String payload) {
		try {
			...
			service.payReservation(reservation.getUsername(),
					reservation.getTitle());
			return ResponseHelper.createResponse(Response.Status.ACCEPTED);
		} catch (ReservationException e) {
			return ResponseHelper.createResponse(
					Response.Status.SERVICE_UNAVAILABLE, e);
		} catch (Exception e) {
			return ResponseHelper.createResponse(
					Response.Status.INTERNAL_SERVER_ERROR, e);
		}
